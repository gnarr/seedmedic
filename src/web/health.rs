...
pub async fn health(State(state): State<AppState>) -> impl IntoResponse {
    let database_ok = state.deps.store.ping().await.is_ok();

    let now = state.deps.clock.now();
    let worker_recent = state.deps.worker_health.last_tick().is_some_and(|last| {
        let threshold = chrono::Duration::from_std(state.health_threshold)
